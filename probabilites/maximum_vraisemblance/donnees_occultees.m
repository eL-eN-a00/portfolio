...
figure('Name','Points situes au voisinage d''un cercle', ...
	   'Position',[0.2*L,0.05*H,0.6*L,0.7*H],'Color',[0.7 0.75 0.85]);
axis equal;
hold on;
set(gca,'FontSize',20);
...
% Occultation de certains points :
thetas = 2*pi*rand(1,2);
[x_donnees_bruitees,y_donnees_bruitees] = occultation_donnees(x_donnees_bruitees,y_donnees_bruitees,theta_donnees_bruitees,thetas); % FONCTION A COMPLETER

% Affichage des donnees bruitees occultees :
plot(x_donnees_bruitees,y_donnees_bruitees,'k*','MarkerSize',5,'LineWidth',2);
axis(bornes);
legend(' Cercle', ...
	   ' Donnees bruitees occultees', ...
	   'Location','Best');
grid on;
