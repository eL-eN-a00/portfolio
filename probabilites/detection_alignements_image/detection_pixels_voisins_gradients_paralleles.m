...
pixels_contour = (G_norme > seuil_norme);

% Pas de pixel de contour sur le bord de l'image (traitement simplifie !) :
pixels_contour([1,nb_lignes],:) = 0;
pixels_contour(:,[1,nb_colonnes]) = 0;

% Affichage d'une petite fleche sur les pixels de contour :
[i,j] = find(pixels_contour);
subplot(1,3,2)
    imagesc(I)
    axis image off;
    colormap gray
    hold on
    quiver(j,i,G_x(pixels_contour),G_y(pixels_contour),'r')  % Attention : x = j et y = i
    title('Gradients de contour','FontSize',15)
...
subplot(1,3,3);
    imagesc(I*0);
    axis image off;
    title('Traitement en cours ...','FontSize',15);
    drawnow;

while any(pixels_contour(:))

	% Pixel de contour dont la norme du gradient est la plus elevee = "germe" :  
	G_norme_seuillee = G_norme.*pixels_contour;
	...
	[E,pixels_contour] = E_recursif(E,pixels_contour,G_somme,i_germe,j_germe,voisins,G_x,G_y,card_max,cos_alpha); % FONCTION A COMPLETER
...
% Affectation d'une couleur specifique a chaque ensemble E :
...
% (la fonction label2rgb donne a chaque ensemble E une couleur differente)
I_resultat = label2rgb(etiquette,'parula',[0.5,0.5,0.5]);
subplot(1,3,3);
    imagesc(I_resultat);
    axis image off;
    title('Ensembles candidats','FontSize',15);

save detection_pixels_voisins_gradients;
