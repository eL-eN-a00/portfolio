...
%
    res = residu(beta);
    J_res = J_residu(beta);
    grad2_ri = [zeros(size(donnees, 2)), donnees(:, 1).*exp(-beta(2).*donnees(:, 1)) ; donnees(:, 1).*exp(-beta(2).*donnees(:, 1)), -beta(1).*beta(2).^2.*exp(-beta(2).*donnees(:, 1))];
    s_beta = sum(res.*grad2_ri);
    H_f = s_beta + J_res'*J_res;
