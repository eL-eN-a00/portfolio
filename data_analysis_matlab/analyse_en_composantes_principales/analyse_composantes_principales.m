...
[C,matrice_passage] = ACP(X);	% FONCTION A CODER
bornes_C = [min(C(:)) max(C(:))];
vecteur_RVB2gris_normalise = matrice_passage(:,1)/norm(matrice_passage(:,1),1);
C1 = C(:,1);
C2 = C(:,2);
C3 = C(:,3);
...
subplot(12,3,35);
axis off
title(['C_1 = ' num2str(vecteur_RVB2gris_normalise(1),'%.3f') '\color{red} R' ...
       '\color{black} + '  num2str(vecteur_RVB2gris_normalise(2),'%.3f') '\color{green} V' ...
       '\color{black} + ' num2str(vecteur_RVB2gris_normalise(3),'%.3f') '\color{blue} B'], ...
      'FontSize',20);
