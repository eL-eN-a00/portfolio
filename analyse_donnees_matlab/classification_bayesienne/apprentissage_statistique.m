...
load donnees_carac;
noms_carac = {'Compacité','Contraste','Texture'};
ind_carac_1 = 1;
ind_carac_2 = 2;
...
figure('Name',['Vraisemblance de la classe ' num2str(num_classe)],...
	'Position',[0.02*L,0.1*H,0.31*L,0.5*H],...
	'Color',[0.7 0.75 0.85]);
...
xlabel(nom_carac_1);
ylabel(nom_carac_2);
legend([dc1, mc1],'Fibromes',['Modèle de la classe ' num2str(num_classe)], ...
...
num_classe = 2;
[mu_2,Sigma_2] = estim_mu_Sigma(X_app(Y_app == num_classe,:));
[grille_carac1,grille_carac2] = meshgrid(graduation_carac_1,graduation_carac_2);
modele_V_melanome = vraisemblances([grille_carac1(:),grille_carac2(:)],mu_2,Sigma_2); % FONCTION A CODER
modele_V_melanome = reshape(modele_V_melanome,length(graduation_carac_2),length(graduation_carac_1));

% Affichage des donnees d'apprentissage de la classe "2" (mélanomes) :
nb_melanome = sum(Y_app == num_classe);
figure('Name',['Vraisemblance de la classe ' num2str(num_classe)],...
	'Position',[0.34*L,0.1*H,0.31*L,0.5*H],...
	'Color',[0.7 0.75 0.85]);
...
title(['Classe ' num2str(num_classe) ' : mélanomes']);
xlabel(nom_carac_1);
ylabel(nom_carac_2);
legend([dc2, mc2],'Melanomes',['Modèle de la classe ' num2str(num_classe)],'Location',...
	'Best','FontSize',15);
zlabel('Vraisemblance','Rotation',90);
set(gca,'FontSize',20);
...
figure('Name','Superposition des deux modèles',...
	'Position',[0.66*L,0.1*H,0.31*L,0.5*H],...
	'Color',[0.7 0.75 0.85]);
...
title('Superposition des deux modèles');
xlabel(nom_carac_1);
ylabel(nom_carac_2);
legend('Modèle de la classe 1','Modèle de la classe 2','Location','Best','FontSize',15);
zlabel('Vraisemblance','Rotation',90);
set(gca,'FontSize',20);
