...
for j = 1:n
	...
	% Estimation des parametres beta par les moindres carres :
	beta = estimation_MC(d,x,bord_inf(:,j));
	y_inf = courbe_bezier(x,[beta_0;beta]);	

	% Estimation des parametres gamma par les moindres carres :
	gamma = estimation_MC(d,x,bord_sup(:,j));
	...
	axis ij;
	grid on;
	title('Silhouettes estimees par deux bords independants');
	legend([hd, hi, hs],...
