...
-- Les commentaires de spécification n'apparaissent qu'ici parce que c'est la spécification du module qui doit spécifier le comportement des sous-programmes pour qu'ils soient coonus à la fois de ceux qui vont utiliser le module (autres programmes et modules) et ceux qui vont l'implanter (implantation du module).
-- Ceci évite aussi d'avoir à écrire deux fois la même chose qu'il faudrait mettre à deux fois en cas de modifications (corrections, évolutions).

-- De nouveau, les contrats sont formalisés dans la spécification et s'appliquent aussi dans l'implantation même s'ils ne sont pas repris.

-- Ce module Piles est générique grâce au mot-clé generic dans la spécification du module, avant package.
-- Ses paramètres de généricité sont : Capacite, une constante entière ; T_Element, un type.
-- Pour pouvoir utiliser ce module Piles, il faut instancier le module générique en donnant une valeur à chaque paramètre de généricité.

-- Il faudrait définir un sous-programme spécifique à Piles seulement dans l'implantation du module.
-- Et c'est là que sa spécification doit être donnée.
