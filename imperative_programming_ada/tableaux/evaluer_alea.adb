...
procedure Evaluer_Alea is
    ...
    is
        package Mon_Alea is
            new Alea (1, Borne);
        use Mon_Alea;
        Frequences : array(1..Borne) of Integer; --Fréquences absolues
        Nombre : Integer;

    begin
        -- Initialiser fréquences
        for I in 1..Borne loop
            Frequences(I) := 0;
        end loop;
        -- Comptabiliser les résultats des tirages aléatoires
        for I in 1..Taille loop
            Get_Random_Number (Nombre);
            Frequences(Nombre) := Frequences(Nombre) + 1;
        end loop;
        -- Calculer les données statistiques
        -- Initialiser les variables statistiques avec la première fréquence
        Min := Frequences(1);
        Max := Min;
        for I in 2..Borne loop
            -- Mettre à jour Min et Max avec Frequences(I)
            if Frequences(I) > Max then
                Max := Frequences(I);
            elsif Frequences(I) < Min then
                Min := Frequences(I);
            else
                null;
            end if;
