...
procedure Exemples_Memoire_Dynamique is
	...
	procedure Illustrer_Memoire_Dynamique_Sans_Free is
		Ptr1 : T_Pointeur_Integer;
	begin
		Ptr1 := new Integer; -- erreur car prompteur non libéré
		Ptr1.all := 5;
         Put_Line ("Ptr1.all = " & Integer'Image (Ptr1.all));
         Free (Ptr1);
	...
	begin
		Ptr1 := new Integer;
	     Ptr2 := new Integer; --pas ouf que 2 pointeurs pointent le mmême endroit
         Ptr1.all := 5;
         Ptr2.all := 5;
		...
		-- XXX A-t-on le droit de manipuler Ptr2.all ?     On a mis le pointeur à null donc la question est : est c que c'est une copie ou ça amène à la même adresse mémoire. Si c'est le 2e cas, erreur
		-- XXX Que se passe-t-il si on exécute le programme avec valgrind ?    erreur ?
		--
	...
	begin
		...
		-- d'appeler Detruire dessus ? si quand même  (dtruit ref a P mais pas P en soit le libere pas)
